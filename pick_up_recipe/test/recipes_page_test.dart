// Правка рецепта прямо со вкладки «Рецепты».
//
// До этой кнопки конструктор открывался только с экрана заваривания: чтобы
// подвинуть помол, надо было сделать вид, что собираешься варить. Кнопка на
// карточке ломается тихо тремя способами, и каждый закрыт тестом ниже:
// её ставят только верхней карточке стопки; она уводит в конструктор с чужим
// рецептом (стопка листается, а виджет держит первую версию); нажатие на неё
// заодно пускает таймер, потому что тап проваливается в карточку под кнопкой.
...
/// Кружки правки в порядке дерева: верхняя карточка стопки — последняя.
final _editButtons = find.byWidgetPredicate(
  (widget) => widget is AppIcon && widget.asset == AppIcons.uiEdit,
);

final _playButtons = find.byWidgetPredicate(
...
void main() {
  ...
  /// Аргументы последнего перехода в конструктор.
  RecipeBuilderRouteArgs builderArgs(_RecordingRouter router) {
    final route = router.pushed.last;
    expect(route.routeName, RecipeBuilderRoute.name);

    return route.args as RecipeBuilderRouteArgs;
  }
  ...
  group('правка рецепта с экрана «Рецепты»', () {
    testWidgets('кружок правки есть у каждой карточки стопки, а не только у верхней',
      ...
      // Три версии — три карточки на экране: верхняя и два края под ней.
      expect(_playButtons, findsNWidgets(3));
      expect(
        _editButtons,
        findsNWidgets(3),
        reason: 'правка появилась не на всех карточках, которые видно',
      );
    });
    ...
    testWidgets('у кружка есть подпись для голосового помощника', (tester) async {
      ...
      expect(find.bySemanticsLabel('Править рецепт'), findsWidgets);

      handle.dispose();
    });

    testWidgets('нажатие уводит в конструктор с рецептом своей карточки',
        (tester) async {
      final router = await pumpRecipes(tester);

      await tester.tap(_editButtons.last);
      await tester.pumpAndSettle();
    ...
    testWidgets('пролистанная стопка отдаёт в конструктор новую верхнюю версию',
      ...
      // Смахиваем верхнюю карточку: наверх поднимается вторая версия.
      await tester.drag(_editButtons.last, const Offset(-150, 0));
      await tester.pumpAndSettle();

      expect(
        router.pushed,
        isEmpty,
        reason: 'кружок перехватил жест, которым листается стопка',
      );
      expect(find.text('версия 2 из 3'), findsOneWidget);

      await tester.tap(_editButtons.last);
      await tester.pumpAndSettle();

      expect(builderArgs(router).recipe.id, 302,
          reason: 'кнопка уводит с рецептом той карточки, на которой стоит');
    });

    testWidgets('кружок правки не мельче соседнего повтора', (tester) async {
      await pumpRecipes(tester);

      // Правка тише повтора по весу, но не по размеру: цель меньше соседней
      // палец находит хуже, чем её сосед, и промах уводит заваривать.
      Size circle(Finder icon) => tester.getSize(
            find.ancestor(of: icon, matching: find.byType(Container)).first,
          );

      final edit = circle(_editButtons.last);
      final play = circle(_playButtons.last);

      expect(edit.width, greaterThanOrEqualTo(play.width));
      expect(edit.height, greaterThanOrEqualTo(play.height));
    });

    testWidgets('нажатие на кружок не запускает заваривание', (tester) async {
      final router = await pumpRecipes(tester);

      await tester.tap(_editButtons.last);
      await tester.pumpAndSettle();

      // Ни таймера, ни экрана заваривания: тап не провалился ни в карточку
      // под кнопкой, ни в соседний кружок повтора.
      expect(
        router.pushed.where((route) => route.routeName == BrewRoute.name),
        isEmpty,
      );
