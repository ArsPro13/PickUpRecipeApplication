...
/// Секунды в «м:сс», а с часами — в «ч:мм:сс».
///
/// Часы появились, когда барабан длительности перестал упираться в пятьдесят
/// девять минут: колд брю на двенадцать часов теперь записывается шагом, и без
/// этой ветки он показывался бы как «720:00». Формат тот же, каким считает
/// оставшееся экран заваривания, — см. `formatDuration` в `brew_page.dart`.
String formatDuration(int seconds) {
  final rest = (seconds % 60).toString().padLeft(2, '0');
  if (seconds >= 3600) {
    final minutes = ((seconds % 3600) ~/ 60).toString().padLeft(2, '0');
    return '${seconds ~/ 3600}:$minutes:$rest';
  }
