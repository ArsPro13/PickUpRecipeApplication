...
// плееру, и он строится из RecipeStep методом fromLegacy.
//
...
class BrewStep {
  ...
  ///
  /// Предикат один на всё приложение: экран заваривания и конструктор обязаны
  /// одинаково понимать, чем такой шаг кончается. Разошлись — и конструктор
  /// рисует поле длительности там, где длительности нет, а плеер глотает шаг
  /// мгновенно.
  bool get endsByUser => untilUser;

  /// Показывать ли длительность шага.
  ///
  /// У шага «пока не скажете сделал» её нет: сколько он продлится, решает
  /// человек. Ноль на месте таймера читается как сбой приложения, а не как
  /// «столько, сколько нужно вам».
  bool get showsDuration => !endsByUser && duration > Duration.zero;

  /// Показывать ли воду шага. Она бывает только у типов, которым её разрешает
  /// схема, и только когда она есть.
