...
class BrewStep {
  ...
  /// Плеер останавливается на этом шаге и ждёт нажатия.
  ///
  /// Уже, чем «шаг заканчивает человек» из step_ending.dart: там к человеку
  /// отнесён и шаг с признаком («пока не осядет пена»), и для конструктора это
  /// верно — время у такого шага не отсчёт, а ориентир. Но останавливать на
  /// нём плеер нельзя: признаки стоят у половины базовых рецептов турки, моки
  /// и сифона, они всю жизнь шли по таймеру, и владелец просил починить ровно
  /// «пока не скажете сделал», а не менять поведение двадцати методов.
  ///
  /// Отсюда два имени вместо одного: разный вопрос — разный ответ.
  bool get waitsForTap => untilUser;
