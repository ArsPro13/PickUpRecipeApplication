...
class GrindReading {
  ...
  /// Готовая строка на месте помола: «14 щ.».
  ///
  /// Ни «примерно», ни имени кофемолки в ней больше нет. Владелец, глядя на
  /// «примерно 12» с «Timemore Chestnut C2» второй строкой: «помол выглядит
  /// так криво; достаточно писать 14 кликов, не нужно писать название
  /// кофемолки». Что деление получено пересчётом и по чьей шкале — это
  /// по-прежнему говорит [hint] на экране заваривания, где под подпись есть
  /// целая строка; в плитке показателей её негде поставить, не изломав её.
  String get label {
    if (!isDivision) return value;
    // «2 круг + 3» и «2A» — тоже законные подписи шкалы, и «щ.» после них
    // выходит бессмыслицей: единица в такой подписи названа своими словами.
    final number = double.tryParse(value.replaceAll(',', '.'));
    return number == null ? value : _words.grinderClicks(value);
  }

  /// Короткая подпись — для плиток показателей, где места на одно слово.
  ///
  /// У помола делением подписи нет: имя кофемолки под числом и было тем, что
  /// ломало плитку. Осталось приглашение выбрать кофемолку — это не имя, а
  /// предложение, и без него на месте помола стоит слово без объяснения.
  String? get caption {
